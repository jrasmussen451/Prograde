...
function ($scope,  $location, OrderSearchCriteria, OrderSearch) {
    ...
    if(store.get('451Cache.OrderSearchType')){
        //var event = store.get('OrderSearchEvent');
        var criteria = store.get('451Cache.OrderSearchType');
        $scope.OrderSearch(null,criteria);
    }
}]);